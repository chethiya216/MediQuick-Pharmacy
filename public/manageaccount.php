 ...
 *
 * If a customer is already logged in, use that customer's ID.
 * If there is no login session, you can open:
 * manageaccount.php?customer_id=1
 *
 * Change the number to the customer_id you want to edit.
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_profile') {
        ...
        } elseif (strlen($firstName) > 50 || strlen($lastName) > 50 || strlen($email) > 100 || strlen($phone) > 20) {
            $errorMessage = 'One or more values are too long.';
        } else {
    ...
    if ($action === 'change_password') {
        ...
        } elseif (password_verify($newPassword, $customer['password_hash'])) {
...
.account-card{
    ...
    margin-bottom:18px;
    overflow:hidden;
    box-shadow:0 5px 22px rgba(22,38,58,.045);
}
...
.account-btn{
    ...
    cursor:pointer;
    transition:.2s ease;
    box-shadow:0 4px 10px rgba(0,163,255,.16);
}
...
.account-toggle[aria-checked="true"]{
    background:var(--mq-blue);
}

.account-toggle[aria-checked="true"]::after{
    left:22px;
}
...
@media(max-width:700px){
    ...
    .account-heading h1{font-size:24px;}

    .info-grid{grid-template-columns:1fr;}

    .account-card-head,
    .account-card-body{padding:17px;}
}
...
@media(max-width:480px){
    ...
    .toggle-area{align-self:flex-end;}
    ...
    .action-link{align-self:flex-end;}
}
</style>
    ...
    <div class="account-container">
        ...
        <aside class="account-sidebar">
            ...
            <nav class="account-nav" aria-label="Account navigation">
                ...
                <a href="manageaccount.php" class="active" aria-current="page">
        ...
        <main class="account-main">
                ...
                <div class="account-card-body">
                    <div class="info-grid">
                        <div class="info-box">
                            ...
                            <div class="info-value" id="fname"><?= htmlspecialchars($customer['first_name']) ?></div>
                        </div>

                        <div class="info-box">
                            <label>Last name</label>
                            <div class="info-value" id="lname"><?= htmlspecialchars($customer['last_name']) ?></div>
                        </div>

                        <div class="info-box">
                            <label>Email address</label>
                            <div class="info-value" id="email"><?= htmlspecialchars($customer['email']) ?></div>
                        </div>

                        <div class="info-box">
                            <label>Phone number</label>
                            <div class="info-value" id="phone"><?= htmlspecialchars($customer['phone'] ?? '') ?></div>
                        </div>
                    </div>
                </div>
            </section>
            ...
            <section class="account-card" id="security">
                ...
                <div class="account-card-body">
                    ...
                    <div class="security-row">
                        <div class="security-left">
                            <div class="security-mini-icon"><i class="fa fa-mobile"></i></div>
                            <div>
                                <div class="label">Two-factor authentication</div>
                                <div class="desc">Add an extra step when signing in</div>
                            </div>
                        </div>

                        <div class="toggle-area">
                            <span class="toggle-status" id="tfa-status">Disabled</span>
                            <button
                                type="button"
            ...
            <section class="account-card">
                ...
                <div class="account-card-body">
                    <div class="action-item">
                        ...
                        <button type="button" class="action-link">Log out all devices</button>
                    </div>
                    ...
                    <div class="action-item">
                        ...
                        <button type="button" class="action-link delete">Delete account</button>
                    </div>
                </div>
            </section>
        </main>
 ...
 <div class="account-modal-box" role="dialog" aria-modal="true">
  ...
  <form method="POST" action="manageaccount.php" id="password-form">
   ...
   <div class="account-modal-body">
    ...
    <div class="form-group"><label>Current password</label><input class="form-control" id="current-password" name="current_password" type="password" autocomplete="current-password" required></div>
    <div class="form-group"><label>New password</label><input class="form-control" id="new-password" name="new_password" type="password" minlength="6" autocomplete="new-password" required></div>
    <div class="form-group"><label>Confirm new password</label><input class="form-control" id="confirm-password" name="confirm_password" type="password" minlength="6" autocomplete="new-password" required></div>
   </div>
...
<style>
.account-modal{display:none;position:fixed;inset:0;z-index:99999;background:rgba(15,28,42,.55);padding:20px;align-items:center;justify-content:center}.account-modal.show{display:flex}.account-modal-box{width:100%;max-width:500px;background:#fff;border-radius:16px;box-shadow:0 20px 60px rgba(0,0,0,.22);overflow:hidden}.account-modal-head{display:flex;align-items:center;justify-content:space-between;padding:18px 22px;border-bottom:1px solid var(--mq-border)}.account-modal-title{display:flex;align-items:center;gap:12px}.modal-icon{width:38px;height:38px;display:flex;align-items:center;justify-content:center;border-radius:10px;background:var(--mq-blue-light);color:var(--mq-blue)}.account-modal-title h3{margin:0;font-size:16px}.account-modal-title p{margin:3px 0 0;font-size:11px;color:var(--mq-muted)}.modal-close{border:0;background:#F4F6F8;color:#718096;width:34px;height:34px;border-radius:9px;cursor:pointer}.account-modal-body{padding:22px}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:15px}.form-group{margin-bottom:15px}.form-group.full{grid-column:1/-1}.form-group label{display:block;margin-bottom:7px;font-size:11px;font-weight:700;color:#687887}.form-control{width:100%;height:43px;border:1px solid #DDE4EA;border-radius:9px;padding:0 12px;font-size:13px;outline:none}.form-control:focus{border-color:var(--mq-blue);box-shadow:0 0 0 3px rgba(0,163,255,.1)}.password-note{font-size:11px;color:var(--mq-muted);margin:0 0 15px}.modal-message{padding:10px 12px;border-radius:8px;font-size:12px;font-weight:600;margin-bottom:15px}.modal-message.error{background:#FFF1F1;color:#C7363B;border:1px solid #FFD5D7}.account-modal-footer{display:flex;justify-content:flex-end;gap:10px;padding:15px 22px;border-top:1px solid var(--mq-border);background:#FAFBFC}.modal-cancel{border:1px solid #DDE4EA;background:#fff;color:#667585;border-radius:8px;padding:9px 15px;font-size:12px;font-weight:700;cursor:pointer}@media(max-width:520px){.account-modal{padding:12px}.form-grid{grid-template-columns:1fr}.form-group.full{grid-column:auto}.account-modal-head,.account-modal-body,.account-modal-footer{padding:16px}}
</style>

<script>
(function(){
 const toggle=document.getElementById('tfa-toggle'),status=document.getElementById('tfa-status');
 if(toggle&&status)toggle.addEventListener('click',function(){const on=this.getAttribute('aria-checked')==='true';this.setAttribute('aria-checked',String(!on));status.textContent=!on?'Enabled':'Disabled';});
 ...
 document.getElementById('password-form')?.addEventListener('submit',function(e){const n=document.getElementById('new-password'),c=document.getElementById('confirm-password');if(n.value!==c.value){e.preventDefault();alert('New password and confirmation do not match.');c.focus();}});
